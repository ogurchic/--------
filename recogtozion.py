...
def execute_command(command):
    commands = {
        ...
        ('добавить заметку', 'напиши новую заметку', "добавь заметку", "добавь новую заметку"):notes.add_note,
        ('удалить заметку', 'удали заметку', "сотри заметку", "стереть заметку"):notes,
        ('который час', 'сколько время', 'подскажи время', "сколько времени"):lambda:whats_time(),
        ('play', 'pause', 'плей', 'пауза', "поставь на паузу", "продолжай играть", 'продолжай', "играть"):lambda:play_pause(),
        ...
        for cmd in cmd_tuple:
            if cmd in command:
                ...
                try: # обработчик исключений
                    ...
                    if cmd in ['прочитай заметки', 'прочитать заметки', "покажи заметки", "открой заметки"]:
                        a = notes.read_notes()
                        print(a)
                        return a
...
def recognition():
    ...
    stop_words = ['stop', 'стоп', 'хватит', 'прекрати', 'кончай', 'спасибо', 'всё, спасибо', 'хватит, спасибо',  
    ...
    while True:
        ...
        try:
            ...
            if text.lower() in stop_words:
                ...
                print(response)
                yield text, response  # Вместо return используем yield
